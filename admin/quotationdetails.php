...
?>
 <div class="page-wrapper cardhead">
    ...
    <button onclick="printquotation()" class="btn submit-button mx-1"><i class="ti ti-printer"></i> Print</button>
    <button onclick="downloadPDF()" class="btn btn-success mx-1"><i class="ti ti-download"></i> PDF</button>
    <a href="editquotation.php?id=<?= $quotation_id ?>" class="btn cancel-button mx-1"><i class="ti ti-pencil"></i> Edit</a>
</div>
            ...
            <div style="display: inline-block; width: 100%;">
                <img src="images/logo.png" style="max-height: 60px; float: right;"/></div> 
            <h2 style="text-align: center; font-size: 32px;">Quotation</h2>
            <table style="font-size: 13px; width:450px; border:1px solid #cccccc; margin-bottom: 25px; padding: 0;" cellspacing="0" cellpadding="0">
                <tbody>
                     <tr>
                        <td style="width:50%; border-right:1px solid #cccccc; border-bottom:1px solid #cccccc; height: 33px; padding-left: 10px;">Quotation No</td>
                        <td style="width:50%; border-right:1px solid #cccccc; border-bottom:1px solid #cccccc; height: 33px; padding-left: 10px; font-weight: 600;" id="companyname">ID100<?= htmlspecialchars($row['id']) ?></td>
                    </tr>
                    <tr>
                        <td style="width:50%; border-right:1px solid #cccccc; border-bottom:1px solid #cccccc; height: 33px; padding-left: 10px;">Company Name</td>
                        <td style="width:50%; border-right:1px solid #cccccc; border-bottom:1px solid #cccccc; height: 33px; padding-left: 10px; font-weight: 600;" id="companyname"><?= htmlspecialchars($row['companyname']) ?></td>
                    </tr>
                    <tr>
                        <td style="width:50%; border-right:1px solid #cccccc; border-bottom:1px solid #cccccc; height: 33px; padding-left: 10px;">Name</td>
                        <td style="width:50%; border-right:1px solid #cccccc; border-bottom:1px solid #cccccc; height: 33px; padding-left: 10px; font-weight: 600;" id="name"><?= htmlspecialchars($row['name']) ?></td>
                    </tr>
                    <tr>
                        <td style="width:50%; border-right:1px solid #cccccc; border-bottom:1px solid #cccccc; height: 33px; padding-left: 10px;">Address</td>
                        <td style="width:50%; border-right:1px solid #cccccc; border-bottom:1px solid #cccccc; height: 33px; padding-left: 10px; font-weight: 600;" id="address"><?= htmlspecialchars($row['address']) ?></td>
                    </tr>
                    <tr>
                        <td style="width:50%; border-right:1px solid #cccccc; border-bottom:1px solid #cccccc; height: 33px; padding-left: 10px;">E-mail</td>
                        <td style="width:50%; border-right:1px solid #cccccc; border-bottom:1px solid #cccccc; height: 33px; padding-left: 10px; font-weight: 600;" id="email"><?= htmlspecialchars($row['email']) ?></td>
                    </tr>
                    <tr>
                        <td style="width:50%; border-right:1px solid #cccccc; border-bottom:0px solid #cccccc; height: 33px; padding-left: 10px;">Phone Number</td>
                        <td style="width:50%; border-right:1px solid #cccccc; border-bottom:0px solid #cccccc; height: 33px; padding-left: 10px; font-weight: 600;" id="phonenumber"><?= htmlspecialchars($row['phonenumber']) ?></td>
                    </tr>
                </tbody>
            </table>
            ...
            <div style="padding-top: 10px; font-size: 14px; padding-right: 28%;">we will be happy to supply any further information you may need and trust that you call on us to fill your order, which will receive our prompt and careful attention.</div>
              ...
              <div style="padding-top: 20px; font-weight: 500; font-size: 16px; text-align: right;"><span style="color: #666666;font-size: 20px;transform: rotate(330deg);display: inline-block;">Electronic Generated Quote</span><br/>Signature :  B.Satish Kumar <br/>
...
<script>
    ...
    $(document).ready(function () {
        const termsVal = $("#termsvalue").val();

        if (termsVal == "1") {
              $("#mobile_terms").text('1 \xa0\xa0\xa0 All Logic Board Service Cary 30day Warranty');
            $("#mobile_terms1").text('2 \xa0\xa0\xa0 Physical damage, Liquid Damage & Power Break issues will not accepected in warranty period.');
            $("#mobile_terms2").text('3 \xa0\xa0\xa0 We pay only 1% GST under Govt composition scheme, GST NO: 36AAFF18494K2Z0.');
            $("#mobile_terms3").text('');
        } else {
            ("#mobile_terms").text('1 \xa0\xa0\xa0 All Logic Board Service Cary 30day Warranty');
            $("#mobile_terms1").text('2 \xa0\xa0\xa0 Physical damage, Liquid Damage & Power Break issues will not accepected in warranty period.');
            $("#mobile_terms2").text('3 \xa0\xa0\xa0 We pay only 1% GST under Govt composition scheme, GST NO: 36AAFF18494K2Z0.');
            $("#mobile_terms3").text('4 \xa0\xa0\xa0 All Payment Modes Accepted, Cheques are subject to Clearance Before Delivery.');
        }
